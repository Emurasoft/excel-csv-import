...
export function SourceInput({ value, onChange }: Props): React.ReactNode {
	...
	switch (value.inputType) {
		case InputType.file:
			...
			break;
		case InputType.text:
			input = textInput(value.text, (text) => onChange({ inputType: InputType.text, file: undefined, text }));
	}
			...
			<Label>
				...
				<Dropdown
					...
					onOptionSelect={(_, { optionValue }) =>
						onChange({ inputType: optionValue as InputType, file: undefined, text: '' })
					}
					id='SourceInput-Dropdown'
				>
					<Option>{InputType.file}</Option>
					<Option>{InputType.text}</Option>
				</Dropdown>
			</Label>
			{input}
		</>
