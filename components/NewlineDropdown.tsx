...
export function NewlineDropdown({ showAutoDetect, value, onChange }: Props): React.ReactNode {
	return (
		<Label>
			...
			>
				{showAutoDetect && <Option value={NewlineSequence.AutoDetect}>{getLabel(NewlineSequence.AutoDetect)}</Option>}
				<Option value={NewlineSequence.CRLF}>{getLabel(NewlineSequence.CRLF)}</Option>
				<Option value={NewlineSequence.CR}>{getLabel(NewlineSequence.CR)}</Option>
				<Option value={NewlineSequence.LF}>{getLabel(NewlineSequence.LF)}</Option>
			</Dropdown>
		</Label>
	);
}
